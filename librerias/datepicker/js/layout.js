	...
	var initLayout = function() {
		...
		$('.inputDate').DatePicker({
			...
			onChange: function(formated, dates){
                ...
                var fecha_act=date_actual.getYear()+"-"+date_actual.getMonth()+"-"+date_actual.getDay();
                $('#fecha_nueva_entrada').DatePicker({
			format:'Y-m-d',
			date: '2010-01-01',
			current: '2010-01-01',
			starts: 1,
			position: 'right',
			onBeforeShow: function(){
				$('#fecha_nueva_entrada').DatePickerSetDate($('#fecha_nueva_entrada').val(), true);
			},
			onChange: function(formated, dates){
				$('#fecha_nueva_entrada').val(formated);
					$('#fecha_nueva_entrada').DatePickerHide();
			}
		});
		var now3 = new Date();
		now3.addDays(-4);
